...
Todo= Manipulate[Grid[{
	{Graphics[{
		...
		If[passaggiList[[1]] == 1,{EdgeForm[{Dashed}],Dashed,Line[{{6,2},{6,0}}]},,],
		If[passaggiList[[1]] == 1,{EdgeForm[{Dashed}],Dashed,Line[{{6,0},{6,-5}}]},,],
		{EdgeForm[{Gray}],White,Polygon[{{6,2},{5,0},{3,1},{4,3}}]},
	
		{FaceForm[{Purple}], Purple,Opacity[passaggiList[[2]]/2],Polygon[{{6,2},{10,0},{12,4},{8,6}}]},
		{FaceForm[{Purple}], Purple,Opacity[passaggiList[[2]]/2],Polygon[{{6,0}, {10,0},{10,-5},{6,-5}}]},
		{FaceForm[{Yellow}], Yellow,Opacity[passaggiList[[3]]/2],Polygon[{{6,2},{5,0},{3,1},{4,3}}]},
		{FaceForm[{Yellow}], Yellow,Opacity[passaggiList[[3]]/2],Polygon[{{5,0}, {6,0},{6,-5},{5,-5}}]},
	
			Black,
			Text[Style[ Subscript[Q,c1] ,"Text",Bold],{4.5,1.5}],
			Text[Style[ Subscript[Q,c2] ,"Text",Bold],{9,3}],
			If[passaggiList[[1]] == 0,Text[Style[ Subscript[Q,"i"] ,"Text",Bold],{7,-2.5}],,],
			If[passaggiList[[1]] == 1,Text[Style[ Subscript[Q,"i"] ,"Text",Bold],{4.8,-2.5}],,],
			If[passaggiList[[1]] == 1,Text[Style[ Subscript[R,"1"] ,"Text",Bold],{5.55,-2.5}],,],
			If[passaggiList[[1]] == 1,Text[Style[ Subscript[R,"2"] ,"Text",Bold],{7.8,-2.5}],,],
	 ...
	 Grid[{
			{If[passaggiList[[1]]==1,Text[Style[Testo1, TextAlignment->Left, LineIndent -> 0]],,]},
			{""},
			{""},
			{If[passaggiList[[2]]==1,Text[Style[Testo2, TextAlignment->Left, LineIndent -> 0]],,]}, 
			{""},
			{""},
			{If[passaggiList[[3]]==1,Text[Style[Testo3, TextAlignment->Left, LineIndent -> 0]],,]}, 
			{""},
			{""},
			{If[passaggiList[[4]]==1,Text[Style[Testo4, TextAlignment->Left]],,]} 
